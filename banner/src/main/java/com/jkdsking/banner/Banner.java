...
import com.jkdsking.banner.listener.OnBannerListener;
import com.jkdsking.banner.listener.OnVideoStateListener;
import com.jkdsking.banner.loader.ViewLoaderInterface;
import com.jkdsking.banner.utils.FileTypeUtil;
import com.jkdsking.banner.view.BannerViewPager;
...
public class Banner extends FrameLayout implements ViewPager.OnPageChangeListener, OnVideoStateListener {
    ...
    private int currentRealItem;//当前真实位置
    ...
    private ViewLoaderInterface imageLoader, videoLoader;
    ...
    //设置图片加载器
    public Banner setImageLoader(ViewLoaderInterface imageLoader) {
        this.imageLoader = imageLoader;
        return this;
    }

    // 设置视频加载器
    public Banner setVideoLoader(ViewLoaderInterface videoLoader) {
    ...
    private void setImageList(List<?> imagesUrl) {
        ...
        for (int i = 0; i <= count + 1; i++) {
            Object url = null;
            if (i == 0) {
                url = imagesUrl.get(count - 1);//实际第一张放置最后一张图片
            } else if (i == count + 1) {
                url = imagesUrl.get(0);//实际最后一张放置第一张图片
            } else {
    ...
    //获取子项目类型 是否是视频
    private boolean getItemTypeIsVideo(Object url) {
        boolean isVideo = false;
        if (url instanceof String) {
            isVideo = FileTypeUtil.isVideoUrl((String) url);
        }
        return isVideo;
    }

    //加载子视图
    private void setViewByLoader(Object url, boolean isVideo) {
        View v = null;
    ...
    class BannerPagerAdapter extends PagerAdapter {
        ...
        public void destroyItem(ViewGroup container, int position, Object object) {
            ...
            if (item instanceof VideoView) {
                VideoView video = (VideoView) item;
                if (video.isPlaying()) {
                    video.stopPlayback();
                }
            }
    ...
    public void onPageSelected(int position) {
        ...
        if (mOnPageChangeListener != null) {
            int currentPosition = toRealPosition(position);
            if (currentRealItem != currentPosition){
                currentRealItem = currentPosition;
                mOnPageChangeListener.onPageSelected(currentRealItem);
            }
        }
